...
def load_embedding(data_path):
    embedding_folder = os.path.join(os.path.dirname(data_path), 'embedding')
...
class DimenGenerator:
    ...
    def generate_dimension_from_seeds(self, seeds):
        ...
        if len(seeds) > 1:
            print(f"Eigene Seeds erkannt ({len(seeds)} Paare). Überspringe automatische Suche.")
            # Wir erstellen direkt das Richtungs-DataFrame aus deinen eigenen Seeds
            direction_group = pd.DataFrame(seed_directions, index=seeds)
            num_directions = len(seeds)
        else:
        ...
        return {
            "note": "manually curated" if len(seeds) > 1 else "generated from seed pairs",
            "seed": seeds,
            "vector": dimension,
